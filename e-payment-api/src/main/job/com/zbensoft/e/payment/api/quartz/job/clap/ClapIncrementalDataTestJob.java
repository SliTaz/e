 ...
 *
 */
public class ClapIncrementalDataTestJob extends ClapIncrementalAbs implements Job {
	
	private static final Logger log = LoggerFactory.getLogger(ClapIncrementalDataTestJob.class);
...
//		}
		isSucc = executeClapSellerUpdateJob();
		if (!isSucc) {
			TASK_LOG.INFO(String.format("%s executeClapSellerUpdateJob is error", key));
			//throw new JobExecutionException(String.format("%s executeClapSellerUpdateJob is fail", key));
		} else {
		...
		isSucc =executeClapDistributionUpdateJob();
		if (!isSucc) {
			TASK_LOG.INFO(String.format("%s executeClapDistributionUpdateJob is error", key));
