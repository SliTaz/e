 ...
 *
 */
public class CxfSignatureConfV2Pro {
	private static final Logger log = LoggerFactory.getLogger(CxfSignatureConfV2Pro.class.getName());
	@Value("${service.url.V2}")
	...
	@Bean
	public BankTranServiceV2 efacturaConsultasClient() {
		JaxWsProxyFactoryBean jaxWsProxyFactory = new JaxWsProxyFactoryBean();
		jaxWsProxyFactory.setServiceClass(BankTranServiceV2.class);
		jaxWsProxyFactory.setAddress(serviceUrl);
		log.info("Consumiendo servicio de " + serviceUrl);
		BankTranServiceV2 bankTranService = (BankTranServiceV2) jaxWsProxyFactory.create();
...
//		properties.put(ConfigurationConstants.SIG_PROP_FILE, "client_signing.properties");
		...
		properties.put(PW_CALLBACK_CLASS, CertificatePasswordHandlerPro.class.getName());
